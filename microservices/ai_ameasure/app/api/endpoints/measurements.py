...
def get_real_time_series_data(
    data_type: str = "displacement", num_points: int = 100, folder_name: str = "01-hokkaido-akan"
    ...
    try:
        ...
        logger.info(
            f"Successfully extracted {len(data)} {data_type} time series points from real data"
        )
        return data
...
def get_real_distribution_data(
    data_type: str = "displacement", folder_name: str = "01-hokkaido-akan"
    ...
    try:
        ...
        logger.info(
            f"Successfully extracted {len(data)} {data_type} distribution bins from real data"
        )
        return data
...
def get_real_scatter_data(
    num_points: int = 200, folder_name: str = "01-hokkaido-akan"
...
def create_dataset(df, df_additional_info):
    ...
    common_columns = [
        ...
        csv_loader.TD_NO,
        csv_loader.STA,
        csv_loader.SECTION_TD,
        csv_loader.FACE_TD,
        csv_loader.DISTANCE_FROM_FACE,
        csv_loader.DAYS_FROM_START,
    ]

    # 追加情報の列定義（Streamlitアプリと同じ）
    additional_info_common_columns = [
        "支保寸法",
        "吹付厚",
        "ﾛｯｸﾎﾞﾙﾄ数",
        "ﾛｯｸﾎﾞﾙﾄ長",
    ...
    def decompose_columns(df, df_additional_info, targets, diff_targets):
        ...
        for i, row in df.iterrows():
            ...
            if "ｻｲｸﾙ" in df_additional_info.columns and csv_loader.CYCLE_NO in row:
                try:
                    matching_index = df_additional_info.index[
                        df_additional_info["ｻｲｸﾙ"] <= row[csv_loader.CYCLE_NO]
                    ].max()
                    if pd.notna(matching_index):
                        df_additional_info_filtered = df_additional_info[
                            df_additional_info.index == matching_index
                        ]
                        filtered_columns = (
                            additional_info_common_columns
                            + additional_info_left_columns
                        ...
                        filtered_columns = [
                            ...
                            if col in df_additional_info_filtered.columns
                        ]

                        if filtered_columns:
                            df_additional_info_filtered = df_additional_info_filtered[
                                filtered_columns
                            ]
                            df.loc[i, df_additional_info_filtered.columns] = (
                                df_additional_info_filtered.values.squeeze()
                            )
                except Exception as e:
        ...
        # ビット列の処理
        bit_columns_in_df = [
            col for col in additional_info_common_bit_columns if col in df_decomposed.columns
        ]
        for c in bit_columns_in_df:
            df_decomposed[f"{c}_bit"] = (~df_decomposed[c].isna()).astype(int)

        # サポートパターンの数値変換
        support_columns_in_df = [
            col for col in additional_info_common_support_columns if col in df_decomposed.columns
        ]
        for c in support_columns_in_df:
            df_decomposed[f"{c}_numeric"] = df_decomposed[c].apply(
                convert_support_pattern_to_numeric
            )
        ...
        x_columns = [
            ...
            if col not in exclude_columns and col != "position_id"
        ]
        ...
        if x_columns:
            # 全てのX列がNaNの行のみを除去
            essential_x_cols = [col for col in x_columns if col in df_decomposed.columns][
                :5
            ]  # 重要な最初の5列のみ
            if essential_x_cols:
    ...
    try:
        # 沈下量データセット作成
        settlement = decompose_columns(
            df,
            df_additional_info,
            csv_loader.SETTLEMENTS,
            csv_loader.DIFFERENCE_FROM_FINAL_SETTLEMENTS,
        )
        # 変位量データセット作成
        convergence = decompose_columns(
            df,
            df_additional_info,
            csv_loader.CONVERGENCES,
            csv_loader.DIFFERENCE_FROM_FINAL_CONVERGENCES,
        )
...
async def get_displacement_series(
    ...
    return schemas.DisplacementSeriesResponse(data=data, unit="mm", measurement_type="displacement")
...
async def get_settlement_series(
    ...
    return schemas.SettlementSeriesResponse(data=data, unit="mm", measurement_type="settlement")
...
async def get_displacement_distribution(
    ...
    return schemas.DisplacementDistributionResponse(
        data=data, bin_size=1, measurement_type="displacement"
    )
...
async def get_settlement_distribution(
    ...
    return schemas.SettlementDistributionResponse(
        data=data, bin_size=1, measurement_type="settlement"
    )
...
async def get_tunnel_scatter(
    ...
    return schemas.TunnelScatterResponse(
        data=data, x_label="切羽からの距離 (m)", y_label="計測経過日数", color_scale="depth"
    )
...
async def get_measurement_files(
    ...
    return schemas.MeasurementFilesResponse(files=files, total_count=len(files))
    ...
    try:
        ...
        if not file_path.exists():
            ...
            file_path = (
                ...
                / "CN_measurement_data"
                / "measurements_A"
                / f"measurements_A_{request.cycleNumber:05d}.csv"
            )

        if not file_path.exists():
            raise HTTPException(
                status_code=404, detail=f"Measurement file not found: {request.cycleNumber}"
            )

        # CSVファイルを処理
        df = csv_loader.process_measurement_file(
            file_path=file_path, max_distance_from_face=request.distanceFromFace, duration_days=90
        )
        ...
        displacement_cols = [
            ...
            if "変位量" in col and "差分" not in col and "ｵﾌｾｯﾄ" not in col
        ]
        ...
        for _, row in df_filtered.iterrows():
            ...
            # 実際の変位量データを取得（最大3つ）
            displacement_a = (
                float(row[displacement_cols[0]])
                if len(displacement_cols) > 0 and pd.notna(row[displacement_cols[0]])
                else 0.0
            )
            displacement_b = (
                float(row[displacement_cols[1]])
                if len(displacement_cols) > 1 and pd.notna(row[displacement_cols[1]])
                else 0.0
            )
            displacement_c = (
                float(row[displacement_cols[2]])
                if len(displacement_cols) > 2 and pd.notna(row[displacement_cols[2]])
                else 0.0
            )
...
async def get_predictions(
    excavationAdvance: float = Query(..., gt=0), distanceFromFace: float = Query(..., ge=0)
...
def generate_additional_info_df(
    cycle_support_csv: Path, observation_of_face_csv: Path
) -> pd.DataFrame:
    """
...
def process_measurement_file(file_path: Path, max_distance_from_face: float = 100) -> pd.DataFrame:
    ...
    """
    df = csv_loader.process_measurement_file(
        file_path=file_path, max_distance_from_face=max_distance_from_face, duration_days=90
    )
...
async def analyze_displacement(
    ...
    try:
        ...
        if not measurements_path.exists():
            raise HTTPException(
                status_code=404, detail=f"Measurements folder not found: {measurements_path}"
            )
        ...
        df_all, dct_df_settlement, dct_df_convergence, dct_df_td, settlements, convergences = (
            generate_dataframes(measurement_a_csvs, max_distance_from_face)
        )
        ...
        if cycle_support_csv.exists() and observation_of_face_csv.exists():
            try:
                df_additional_info = generate_additional_info_df(
                    cycle_support_csv, observation_of_face_csv
                )
        ...
        stats = {
            ...
            "distance_range": {
                "min": (
                    safe_float(df_all[DISTANCE_FROM_FACE].min())
                    if DISTANCE_FROM_FACE in df_all.columns
                    else None
                ),
                "max": (
                    safe_float(df_all[DISTANCE_FROM_FACE].max())
                    if DISTANCE_FROM_FACE in df_all.columns
                    else None
                ),
            },
            "days_range": {
                "min": (
                    safe_float(df_all[DAYS_FROM_START].min())
                    if DAYS_FROM_START in df_all.columns
                    else None
                ),
                "max": (
                    safe_float(df_all[DAYS_FROM_START].max())
                    if DAYS_FROM_START in df_all.columns
                    else None
                ),
            },
        ...
        if dct_df_convergence:
            ...
            for name, values in dct_df_convergence.items():
                if values:
                    ...
                    sns.histplot(data, bins=20, alpha=0.5, label=name, kde=True)
            plt.title("変位量分布")
            plt.xlabel("変位量 (mm)")
            plt.ylabel("頻度")
            plt.legend()
        ...
        if dct_df_td and convergences:
            ...
            for name, df in dct_df_td.items():
                if not df.empty and TD_NO in df.columns:
                    # 各収束値の平均を計算
                    conv_mean = (
                        df[convergences].mean(axis=1)
                        if all(c in df.columns for c in convergences)
                        else pd.Series()
                    )
        ...
        if dct_df_td and settlements:
            ...
            for name, df in dct_df_td.items():
                if not df.empty and TD_NO in df.columns:
                    # 各沈下値の平均を計算
                    settle_mean = (
                        df[settlements].mean(axis=1)
                        if all(s in df.columns for s in settlements)
                        else pd.Series()
                    )
...
async def get_scatter_plot_data(
    ...
    max_distance_from_face: float = Query(default=100.0, gt=0, description="切羽からの最大距離"),
    plot_type: str = Query(
        default="convergences", description="プロットタイプ: convergences or settlements"
    ),
) -> Dict[str, Any]:
    """
    散布図用のデータを生成
    ...
    try:
        ...
        if plot_type == "convergences":
            ...
            raise HTTPException(
                status_code=400, detail="plot_type must be 'convergences' or 'settlements'"
            )
        ...
        return {
            ...
            "color_range": {"min": min_value, "max": max_value},
            "plot_type": plot_type,
        }
...
async def debug_dataset(request: MLDatasetRequest) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        # 正しいパス構築
        input_folder = (
            settings.DATA_FOLDER / request.folder_name / "main_tunnel" / "CN_measurement_data"
        )
        cycle_support_csv = input_folder / "cycle_support" / "cycle_support.csv"
        observation_of_face_csv = input_folder / "observation_of_face" / "observation_of_face.csv"
        ...
        if cycle_support_csv.exists() and observation_of_face_csv.exists():
            ...
            available_diff_settlements = [
                col for col in DIFFERENCE_FROM_FINAL_SETTLEMENTS if col in df_all.columns
            ]
            available_diff_convergences = [
                col for col in DIFFERENCE_FROM_FINAL_CONVERGENCES if col in df_all.columns
            ]
    ...
    for col in df_clean.columns:
        ...
        if df_clean[col].dtype in ["float64", "float32", "int64", "int32"]:
            ...
            df_clean[col] = df_clean[col].where(
                (df_clean[col] >= -1e308) & (df_clean[col] <= 1e308), np.nan
            )
            # NaNをNoneに変換（JSON対応）
            df_clean[col] = df_clean[col].where(pd.notna(df_clean[col]), None)
        elif "datetime" in str(df_clean[col].dtype) or "timestamp" in str(df_clean[col].dtype):
...
async def make_dataset(request: MLDatasetRequest):
    """
    計測データから機械学習用のデータセットを作成
    訓練用とテスト用に分割し、指定したフォーマットで保存
    """
    # 安全なフォールバック：常に空のレスポンスを準備
    safe_response = {"settlement_data": [], "convergence_data": []}

    try:
        logger.info(
            f"make_dataset called with folder_name: {request.folder_name}, max_distance: {request.max_distance_from_face}"
        )
        from app.core.dataframe_cache import get_dataframe_cache
        ...
        logger.info(
            f"Additional info shape: {df_additional_info.shape if not df_additional_info.empty else 'empty'}"
        )
        ...
        try:
            if isinstance(settlement_data, tuple) and len(settlement_data) == 3:
                ...
                logger.info(
                    f"Settlement DataFrame shape: {df_s.shape if not df_s.empty else 'empty'}"
                )
                logger.info(
                    f"Settlement X columns: {x_cols_s[:5] if x_cols_s else 'None'}"
                )  # 最初の5列のみログ
        ...
        try:
            if isinstance(convergence_data, tuple) and len(convergence_data) == 3:
                ...
                logger.info(
                    f"Convergence DataFrame shape: {df_c.shape if not df_c.empty else 'empty'}"
                )
                logger.info(
                    f"Convergence X columns: {x_cols_c[:5] if x_cols_c else 'None'}"
                )  # 最初の5列のみログ
