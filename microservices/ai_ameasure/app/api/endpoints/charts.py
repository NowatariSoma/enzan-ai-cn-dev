    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as tmp:
            ...
            return ChartResponse(
                success=True, message="Chart generated successfully", file_path=tmp.name
            )
...
async def draw_histogram_chart(request: HistogramChartRequest):
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as tmp:
            ...
            return ChartResponse(
                success=True, message="Histogram generated successfully", file_path=tmp.name
            )
...
async def download_chart(file_path: str):
    ...
    return FileResponse(
        path=file_path, media_type="image/png", filename=os.path.basename(file_path)
    )
    ...
    try:
        ...
        if not df_all.empty:
            response_data["statistics"] = {
                ...
                "distance_range": [
                    float(df_all[DISTANCE_FROM_FACE].min()),
                    float(df_all[DISTANCE_FROM_FACE].max()),
                ],
                "available_distances": list(dct_df_td.keys()),
            }

        return DataResponse(success=True, message="Data retrieved successfully", data=response_data)
...
async def get_processed_dataframe(request: DataRequest):
    ...
    try:
        ...
        result = {
            ...
            "shape": {"rows": len(df_all), "columns": len(df_all.columns)},
        }
        ...
        return DataResponse(
            success=True, message="Processed dataframe retrieved successfully", data=result
        )
...
async def draw_heatmap_chart(request: HeatmapRequest):
    ...
    try:
        ...
        if not cached_data:
            raise HTTPException(
                status_code=404, detail=f"Failed to load data for folder: {request.folder_name}"
            )
    ...
    try:
                ...
                if combined_df.empty:
                    ...
                    if common_cols:
                        combined_df = pd.concat(
                            [combined_df[common_cols], df_c[common_cols]], ignore_index=True
                        )
        ...
        for i, feature_y in enumerate(features):
            for j, feature_x in enumerate(features):
                heatmap_data.append(
                    {"x": feature_x, "y": feature_y, "value": correlation_matrix.iloc[i, j]}
                )
        ...
        return DataResponse(
            ...
            data={
                ...
                "shape": {"rows": len(combined_df), "columns": len(combined_df.columns)},
            },
        )
