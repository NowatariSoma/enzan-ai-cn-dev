...
async def process_measurement_file(
    ...
    try:
        ...
        if not file_path.is_absolute():
            ...
            file_path = settings.DATA_FOLDER / request.folder_name / "main_tunnel" / "CN_measurement_data" / "measurements_A" / request.file_path
...
async def get_available_columns(
    ...
    try:
        # ファイルパスを構築
        file_path = settings.DATA_FOLDER / folder_name / "main_tunnel" / "CN_measurement_data" / "measurements_A" / file_name
...
async def extract_specific_data(
    ...
    try:
        ...
        if not full_path.is_absolute():
            full_path = settings.DATA_FOLDER / folder_name / "main_tunnel" / "CN_measurement_data" / "measurements_A" / file_path
        ...
        for col in columns:
            if pd.api.types.is_numeric_dtype(extracted_df[col]):
                stats[col] = {
                    "mean": float(extracted_df[col].mean()) if not extracted_df[col].isna().all() else None,
                    "min": float(extracted_df[col].min()) if not extracted_df[col].isna().all() else None,
                    "max": float(extracted_df[col].max()) if not extracted_df[col].isna().all() else None,
                    "std": float(extracted_df[col].std()) if not extracted_df[col].isna().all() else None
                }
