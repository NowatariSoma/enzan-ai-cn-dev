...
def generate_dataframes(measurement_a_csvs: List[Path], max_distance_from_face: float):
    ...
    for distance in distances:
        if distance <= max_distance_from_face:
            ...
            if mask.any():
                dct_df_settlement[distance] = (
                    df_all.loc[mask, settlements].values.flatten() if settlements else []
                )
                dct_df_convergence[distance] = (
                    df_all.loc[mask, convergences].values.flatten() if convergences else []
                )
                dct_df_td[distance] = (
                    df_all.loc[mask, "TD"].values if "TD" in df_all.columns else []
                )
...
def simulate_displacement(
    ...
    daily_advance: float = None,
    distance_from_face: float = None,
    ...
    df_all, _, _, _, settlements, convergences = generate_dataframes(
        [Path(a_measure_path)], max_distance_from_face
    )
    ...
    if daily_advance and distance_from_face:
        ...
        df_all_new["切羽からの距離"] = df_all.iloc[0]["切羽からの距離"] + daily_advance * pd.Series(
            range(max_record)
        )

        df_all = pd.concat(
            [df_all_actual, df_all_new[distance_from_face <= df_all_new["切羽からの距離"]]],
            ignore_index=True,
        ).reset_index()
    ...
    for i, ((df, x_columns, y_column), target) in enumerate(
        ...
        try:
            ...
            if recursive and daily_advance and distance_from_face:
                ...
                df.loc[df["切羽からの距離"] > distance_from_face, _y_column] = _y_hat[
                    df["切羽からの距離"] > distance_from_face
                ]
...
def list_folders() -> List[str]:
    ...
    return (
        sorted(folders) if folders else ["01-hokkaido-akan", "02-tohoku-sendai", "03-kanto-tokyo"]
    )
...
async def analyze_displacement(
    ...
    try:
        ...
        df_all, dct_df_settlement, dct_df_convergence, dct_df_td, settlements, convergences = (
            generate_dataframes(measurement_a_csvs, request.max_distance)
        )
        ...
        # データセット作成
        settlement_data, convergence_data = create_dataset(
            df_all, df_additional_info or pd.DataFrame()
        )
        ...
        for _, row in df_all.iterrows():
            ...
            # 実測値
            displacement_a = (
                float(row["変位量A"]) if "変位量A" in row and pd.notna(row["変位量A"]) else 0.0
            )
            displacement_b = (
                float(row["変位量B"]) if "変位量B" in row and pd.notna(row["変位量B"]) else 0.0
            )
            displacement_c = (
                float(row["変位量C"]) if "変位量C" in row and pd.notna(row["変位量C"]) else 0.0
            )
        ...
        return schemas.DisplacementAnalysisResponse(
            ...
            feature_importance_a=(
                [{"feature": k, "importance": v} for k, v in feature_importances[0].items()]
                if len(feature_importances) > 0
                else []
            ),
            feature_importance_b=(
                [{"feature": k, "importance": v} for k, v in feature_importances[1].items()]
...
async def get_available_folders() -> List[str]:
    ...
    # フォルダがない場合はモックデータを返す
    if not folders:
        folders = ["01-hokkaido-akan", "02-tohoku-sendai", "03-kanto-tokyo"]
...
async def get_cycle_files(folder_name: str) -> List[str]:
    ...
    try:
        ...
        return (
            sorted(csv_files) if csv_files else [f"measurements_A_{i:05d}.csv" for i in range(1, 6)]
        )
