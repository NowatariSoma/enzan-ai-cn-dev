...
try:
    from app.displacement_original import (
        CONVERGENCES,
        CYCLE_NO,
        DATE,
        DAYS_FROM_START,
        ...
        STA,
        TD_NO,
        proccess_a_measure_file,
    )
except:
    from displacement_original import (
        proccess_a_measure_file,
        DATE,
        CYCLE_NO,
        SECTION_TD,
...
def drop_unnecessary_columns(df_distance, index, df_additional_info, x_columns):
    ...
    df_data_only = df_data_only.loc[
        :, ~df_data_only.columns.str.contains("ＳＴＡ*|断面番号*|St№*|年|月|日|測点*")
    ]
    df_data_only = df_data_only.sort_values(by=SECTION_TD)
    df_data_only = df_data_only[df_data_only[SECTION_TD] > 0]
    df_data_only = df_data_only.drop(
        columns=[SECTION_TD, "支保№", "予備", "坑口からの距離"], errors="ignore"
    )
...
def create_dataset(df, df_additional_info):
    ...
    def decompose_columns(df, df_additional_info, targets, diff_targets):
        ...
        for i, row in df.iterrows():
            matching_index = df_additional_info.index[
                df_additional_info["ｻｲｸﾙ"] <= row[CYCLE_NO]
            ].max()
        ...
        for i, (a, b) in enumerate(
            ...
            _df.rename(
                columns={src: tgt for src, tgt in zip(b, additional_info_columns)}, inplace=True
            )
            _df.loc[:, "position_id"] = i
            df_decomposed = pd.concat([df_decomposed, _df], axis=0, ignore_index=True)

        df_decomposed[[f"{c}_bit" for c in additional_info_common_bit_columns]] = (
            ~df_decomposed[additional_info_common_bit_columns].isna()
        ).astype(int)
        ...
        x_columns = df_decomposed.drop(
            columns=[DATE, CYCLE_NO, TD_NO, STA, SECTION_TD, FACE_TD, diff_targets[0][:-1]]
        ).columns.to_list()
        ...
        x_columns = [
            ...
            if col
            not in additional_info_common_bit_columns + additional_info_common_support_columns
        ]
    ...
    settlement = decompose_columns(
        df, df_additional_info, SETTLEMENTS, DIFFERENCE_FROM_FINAL_SETTLEMENTS
    )
    convergence = decompose_columns(
        df, df_additional_info, CONVERGENCES, DIFFERENCE_FROM_FINAL_CONVERGENCES
    )
...
def analyze_displacement(
    input_folder, output_path, model_paths, model, max_distance_from_face, td=None
):
    ...
    draw_charts_histram_displace(
        os.path.join(output_path, "settle_hist2.png"), dct_df_settlement, settlements
    )
    draw_charts_histram_displace(
        os.path.join(output_path, "conv_hist2.png"), dct_df_convergence, convergences
    )
    ...
    def process_each(model, df, x_columns, y_column, td):
        try:
            draw_heatmap(
                os.path.join(output_path, f"heatmap_{y_column}.png"), df[x_columns + [y_column]]
            )
            if td is None:
            ...
            draw_scatter_plot(
                os.path.join(output_path, f"scatter_{y_column}_train.png"),
                df_train[y_column],
                df_train["pred"],
                label="Train Data",
            ...
            draw_scatter_plot(
                os.path.join(output_path, f"scatter_{y_column}_validate.png"),
                df_validate[y_column],
                df_validate["pred"],
                label="Validate Data",
            ...
            draw_feature_importance(
                os.path.join(output_path, f"feature_importance_{y_column}.png"), model, x_columns
            )
...
def main():
    ...
    parser.add_argument(
        "--max-distance-from-face",
        type=float,
        default=100,
        help="max distance from face to consider.",
    )
    ...
    analyze_displacement(
        args.input_folder, args.output_path, args.max_distance_from_face, td=args.td
    )
