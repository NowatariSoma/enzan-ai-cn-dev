...
class PredictionEngine:
    ...
    def __init__(self):
        self.model_manager = ModelManager(settings.DATA_FOLDER.parent / "microservices" / "ai_ameasure" / "config" / "models.yaml")
        self.csv_loader = CSVDataLoader()
        
        # 特徴量・ターゲット列の定義
        self.feature_columns = ['TD(m)', '切羽TD', '実TD', 'ｻｲｸﾙNo']
        self.settlement_columns = ['沈下量1', '沈下量2', '沈下量3', '沈下量4', '沈下量5', '沈下量6', '沈下量7']
        self.convergence_columns = ['変位量A', '変位量B', '変位量C', '変位量D', '変位量E', '変位量F', '変位量G', '変位量H', '変位量I']
    ...
    def train_model(
        ...
        try:
            ...
            if y_train.shape[1] > 1:
                train_score = np.mean([r2_score(y_train.iloc[:, i], train_pred[:, i]) 
                                     for i in range(y_train.shape[1])])
                test_score = np.mean([r2_score(y_test.iloc[:, i], test_pred[:, i]) 
    ...
    def simulate_displacement(
        ...
        try:
            ...
            for day in range(prediction_days):
                ...
                settlement_pred = {}
                try:
                    settlement_result = self.predict(use_models["settlement"], features, folder_name)
                    settlement_pred = {
                        "series1": settlement_result["prediction"][0] if isinstance(settlement_result["prediction"], list) else settlement_result["prediction"],
                        "series2": settlement_result["prediction"][1] if isinstance(settlement_result["prediction"], list) and len(settlement_result["prediction"]) > 1 else 0.0,
                        "series3": settlement_result["prediction"][2] if isinstance(settlement_result["prediction"], list) and len(settlement_result["prediction"]) > 2 else 0.0
                    }
                except Exception as e:
                ...
                convergence_pred = {}
                try:
                    convergence_result = self.predict(use_models["convergence"], features, folder_name)
                    convergence_pred = {
                        "seriesA": convergence_result["prediction"][0] if isinstance(convergence_result["prediction"], list) else convergence_result["prediction"],
                        "seriesB": convergence_result["prediction"][1] if isinstance(convergence_result["prediction"], list) and len(convergence_result["prediction"]) > 1 else 0.0,
                        "seriesC": convergence_result["prediction"][2] if isinstance(convergence_result["prediction"], list) and len(convergence_result["prediction"]) > 2 else 0.0
                    }
                except Exception as e:
                ...
                try:
                    if "final_settlement" in use_models:
                        final_sett_result = self.predict(use_models["final_settlement"], features, folder_name)
                        final_settlement = final_sett_result["prediction"] if isinstance(final_sett_result["prediction"], (int, float)) else final_sett_result["prediction"][0]
                except:
                    pass
                
                try:
                    if "final_convergence" in use_models:
                        final_conv_result = self.predict(use_models["final_convergence"], features, folder_name)
                        final_convergence = final_conv_result["prediction"] if isinstance(final_conv_result["prediction"], (int, float)) else final_conv_result["prediction"][0]
                except:
                    pass
            ...
            summary = {
                ...
                "final_td": current_td,
                "max_settlement": max([max(d["settlement_prediction"].values()) for d in simulation_data]),
                "max_convergence": max([max(d["convergence_prediction"].values()) for d in simulation_data])
            }
